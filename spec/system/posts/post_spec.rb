...
RSpec.describe 'Posts', type: :system do
  describe 'レビューのCRUD' do
    ...
    let(:author) { create(:author) }
      ...
      #     login_as(me)
      #     click_on('レビューリスト')
      #     expect(current_path).to eq(books_path), 'ヘッダーのリンクからレビュー一覧画面へ遷移できません'
      #   end
        ...
        context 'レビューが一件もない場合' do
          it '何もない旨のメッセージが表示されること' do
            login_as(me)
            visit books_path
            expect(page).to have_content('読みたいリストがありません'), 'レビューが一件もない場合、「読みたいリストがありません」というメッセージが表示されていません'
          end
        end
